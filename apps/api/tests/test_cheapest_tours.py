...
async def test_cheapest_tours_topn_distinct_ordered_and_stars(
    ...
    await _seed_price(db_session, hotel_id=h_cheap, operator_id=op, check_in=ci, price_uah=20000, observed_at=db_now)
    await _seed_price(db_session, hotel_id=h_mid, operator_id=op, check_in=ci, price_uah=30000, observed_at=db_now)
    await _seed_price(db_session, hotel_id=h_exp, operator_id=op, check_in=ci, price_uah=40000, observed_at=db_now)
    await _seed_price(db_session, hotel_id=h_fourth, operator_id=op, check_in=ci, price_uah=50000, observed_at=db_now)
    # 2-star is the absolute cheapest but must be filtered out by min_stars.
    await _seed_price(db_session, hotel_id=h_two_star, operator_id=op, check_in=ci, price_uah=5000, observed_at=db_now)
...
async def test_cheapest_tours_per_hotel_cheapest_offer(db_session: AsyncSession) -> None:
    ...
    await _seed_price(db_session, hotel_id=hotel, operator_id=op, check_in=db_today + timedelta(days=20), price_uah=33000, observed_at=db_now)
    await _seed_price(db_session, hotel_id=hotel, operator_id=op, check_in=db_today + timedelta(days=21), price_uah=27000, observed_at=db_now, meal_plan="BB")
    await _refresh_mv(db_session)
...
async def test_cheapest_tours_freshness_gate_drops_stale(db_session: AsyncSession) -> None:
    ...
    await _seed_price(db_session, hotel_id=stale_hotel, operator_id=op, check_in=ci, price_uah=10000, observed_at=db_now - timedelta(hours=48))
    await _seed_price(db_session, hotel_id=fresh_hotel, operator_id=op, check_in=ci, price_uah=25000, observed_at=db_now)
    await _refresh_mv(db_session)
...
async def test_cheapest_tours_future_window_filter(db_session: AsyncSession) -> None:
    ...
    await _seed_price(db_session, hotel_id=too_soon, operator_id=op, check_in=db_today + timedelta(days=1), price_uah=9000, observed_at=db_now)
    await _seed_price(db_session, hotel_id=in_window, operator_id=op, check_in=db_today + timedelta(days=30), price_uah=22000, observed_at=db_now)
    await _seed_price(db_session, hotel_id=too_far, operator_id=op, check_in=db_today + timedelta(days=91), price_uah=8000, observed_at=db_now)
    await _refresh_mv(db_session)
