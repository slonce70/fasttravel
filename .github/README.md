...
> **Local-first phase.** На даний момент `docker compose up -d` локально
> працює без жодних GitHub секретів. Workflows які потребують VPS / R2 (deploy,
> backup, sitemap) мають **graceful skip** — вони запускаються за розкладом,
...
|---|---|---|---|
| `ci.yml` | PR + push до `main` | Lint, type check, тести (API + scheduler), build фронту, docker compose smoke build | _(none)_ optionally `CODECOV_TOKEN` |
| `deploy-api.yml` | Push до `main` (зміни в `apps/*` / `infra/*` / compose) + manual | Build і push образів до GHCR, SSH у VPS, alembic migration, recreate backend сервісів, health check | VPS SSH + (опц.) webhook |
| `daily-backup.yml` | Cron `0 1 * * *` (01:00 UTC) + manual | SSH у VPS, `pg_dump → gzip → R2`, retain 30 last | VPS SSH + R2 |
| `nightly-sitemap.yml` | Cron `0 2 * * *` (02:00 UTC) + manual | **MVP placeholder.** Згенерувати sitemap.xml у API контейнері і залити на R2 / Pages. Реальна реалізація після `apps/api/scripts/generate_sitemap.py` | VPS SSH + R2 |
...
| Nightly sitemap | ~1 хв (placeholder) | 30 | ~30 |
...
> Залежить від `permissions: { packages: write }` у workflow — вже стоїть.

#### Sitemap (`nightly-sitemap.yml`)
Reuse: `DEPLOY_SSH_*` + `R2_*`, плюс окремий `R2_SITEMAP_BUCKET` (може дорівнювати
бакету, який роздає Cloudflare Pages).
...
1. **Замінити `@YOUR_USERNAME` у `CODEOWNERS`** на свій handle.
2. У `Settings → Branches → main`:
   - Enable "Require a pull request before merging".
   - Enable "Require status checks to pass" → додати `backend (api) lint + test`, `scheduler lint + test`, `bot lint`, `frontend lint + build`, `docker compose validate + build`.
   - Enable "Require review from Code Owners".
3. У `Settings → Actions → General`:
   - Workflow permissions: `Read and write` (потрібно для GHCR push з deploy-api).
4. Поки немає VPS — deploy/backup/sitemap workflows будуть no-op'ити з логом
...
CI зараз працює без `poetry.lock` / `pnpm-lock.yaml` (їх ще не закомічено).
Як тільки вони з'являться:

- У `ci.yml` повернути `cache: poetry` / `cache: pnpm` в `setup-python` / `setup-node`.
- `pnpm install --no-frozen-lockfile` → `pnpm install --frozen-lockfile`.

Це дасть швидші CI runs і guaranteed reproducibility.
